...
var cGameState = function (level) {
	...
	this.emptyCells = function() {
		logger.log("Entrando en celdas vacías");
		var available_list = [];
		for (var row = 0; row < 3; row++) {
			for (var col = 0; col < 3; col++) {
	...
	this.markCell = function(row, col) {
		if(this.isCellMarked(row, col)) {
			logger.log("La celda ya está marcada");
			return false;
		}
	...
	this.isGameOver = function() {
		logger.log("Entro en isGameOver : " + this.TOTAL_MOVES);
		...
		if( this.GAME_RESULT !== this.RESULTS.incomplete ) {
			logger.log("Well, a result is already reached");
			return true;
		}

		// make sure there is no <= there, elese the gameover is not coming after 5 moves. yuck!
		if(this.TOTAL_MOVES < 5) {
			logger.log("Muy pocos movimientos, el juego no puede terminar todavía");
		...
		if(game_status) {
			logger.log("EL GANADOR DEL JUEGO ES " + this.TURN);
			...
			logger.log("Todos los pasos están agotados, es un empate");
	...
	this.getFinishMoves = function(player) {
		...
		if(this.TOTAL_MOVES < 3) {
			logger.log("Muy pocos movimientos");
			this.GAME_STATUS = this.RESULTS.incomplete ; 
			return false;
		}
