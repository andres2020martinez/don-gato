...
UI.drawBoard = function () {
	...
	var gameboard_code = '\
			...
			<div id="x-won" class="container">' + svg_x + '<span>GANADOR</span></div>\
			<div id="o-won" class="container">' + svg_o + '<span>GANADOR</span></div>\
			<div id="tie"   class="container">' + svg_xo + '<span>Empate</span></div>\
	...
	document.getElementById("messageboard").innerHTML = "<span><br>¡¡Hola!! Elige un bloque.</span>";

	document.getElementById("score-x").innerHTML = "Humano [X] : " + score_human;
	document.getElementById("score-o").innerHTML = "IA [O] : " + score_robot;
	...
	if (alreadySledgedFlag === 0) {
		...
		if (winLoseStreak.valueOf() === 2) {
			setTimeout(function () {
				...
				if ($('#facil').hasClass('active')) {
					$('#facil').removeClass('active');
					$('#medio').addClass('active');
					alert("Cambiando de facil a medio");
					winLoseStreak = 0;
				} else if ($('#medio').hasClass('active')) {
					$('#dificil').addClass('active');
					$('#medio').removeClass('active');
					alert("Cambiando de medio a dificil");
					winLoseStreak = 0;
				} 

			}, 1000);

			alreadySledgedFlag = 0;
		}

		if (winLoseStreak.valueOf() === -2) {
			setTimeout(function () {
				//document.getElementById('sledgingModalId').innerHTML = "";
				//$('#sledgingModal').modal('show');

				if ($('#medio').hasClass('active')) {
					$('#medio').removeClass('active');
					$('#facil').addClass('active');
					alert("Cambiando de medio a facil");
					winLoseStreak = 0;
				} else {
					$('#dificil').removeClass('active');
					$('#medio').addClass('active');
					alert("Cambiando de dificil a medio");
					winLoseStreak = 0;
				}
	...
	switch (parGameState.GAME_RESULT) {
		case parGameState.RESULTS.incomplete: {
			if (parGameState.TURN === parGameState.SYMBOL.robot) {
				...
				document.getElementById("messageboard").innerHTML = "<br>Es turno de las O";
			}
				...
				document.getElementById("messageboard").innerHTML = "<br>Es turno de las X";
			}
		}
			break;
		...
		case parGameState.RESULTS.playerXWon: {
			document.getElementById("messageboard").innerHTML = "<br> Fin del Juego";
			document.getElementById("score-o").classList.add('focus-score-o');
			document.getElementById("score-x").classList.add('focus-score-x');
		...
		default: {
			...
			document.getElementById("messageboard").innerHTML = "<br>¿Qué diablos acaba de pasar? eh";
			document.getElementById("score-o").classList.add('focus-score-o');
			document.getElementById("score-x").classList.add('focus-score-x');
		}
	}
